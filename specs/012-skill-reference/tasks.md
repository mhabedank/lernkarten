...
- [ ] T007 🛡️ **row 2 (guard)** Add `test_the_docs_requirements_are_not_a_runtime_dependency` to `tests/test_docsite_layout.py`: parse the package names out of `requirements-docs.txt`, assert none appears in `REQUIREMENTS` in `scripts/deps.py`, and that nothing under `bin/` or `scripts/` imports one (FR-003, FR-005, SC-009). It is green on the first run — it guards FR-003/FR-005 against a *later* feature. Label it a guard in the docstring, in the shape `tests/test_landing_page.py:471` already uses
...
- [ ] T014 🔴 [US2] **row 5** Create `tests/test_build_docs.py` with a module-level skip that names `requirements-docs.txt` — mirror the engine skip in `tests/test_e2e.py` (FR-023, SC-006) — and `test_the_build_exits_zero_and_writes_an_index`: run `python3 scripts/build_docs.py --site <tmp>` as a subprocess, assert exit 0 and an `index.html` under `<tmp>/docs/`. Red on its assertion, not on a skip (T008 is why)
- [ ] T015 [US2] **row 5 green (1/3)** Write `scripts/build_docs.py` — new module, module docstring in the established style (what it does, the command that invokes it, why it exists). It: checks the docs requirements are importable and, if not, exits non-zero with one line naming `pip install -r requirements-docs.txt` (the courtesy `scripts/deps.py` and `scripts/engine.py` already extend); runs Sphinx into `<site>/docs/`; performs the three `shutil.copy2` copies and `Path.touch()` of `.nojekyll` per plan § *The `_site` assembly, stated once*; prints the path to open. One option, `--site <dir>`, defaulting so that the no-argument form is the FR-019 command. **It imports no local module** — it is a new leaf of Principle VI's graph (FR-019, FR-038)
...
- [ ] T045 [P] [US1] **row 18 green** Retarget `README.md`'s three references — `docs/workflow.md` (line 79), `docs/design.md` (274), `docs/testing.md` (288) — at the published pages. Accepted cost, recorded in FR-042: `check_docs.check_links` skips `http` targets, so those three leave its file-system coverage; `REQUIRED_FILES` still requires the files to exist
