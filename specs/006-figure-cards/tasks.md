...
Phase 11 added (T114–T124); T057 and T058 reopened — they pass against a fixture
that shares none of the properties of the real web.
...
## Phase 11: BUG-008 — `fetch` reaches the real web

**Purpose**: `fetch` could not reach 28 % of real picture URLs, and the rejected
set was disproportionately the diagrams the feature exists for. Two defects that
mask each other, so both or neither. See [bugs/BUG-008.md](./bugs/BUG-008.md).

**Independent Test**: against a server that imitates the real web — extensionless
paths, a 403 for a request with no `User-Agent`, an HTML error page behind a
`.png` URL — all four shapes behave (SC-011).
