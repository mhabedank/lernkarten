...
2. **4 × 4 is exactly A8** — the next standard size down, and the only smaller grid with a purchasable box. At the default 5 mm margin it cuts to 50 × 71.75 mm, which drops into an A8 box with a little slack.
...
5. **Given** the 29 demo cards built at A8, **When** the build reports overflow, **Then** **none of them is reported**. Measured against the real engine: every demo card fits at A8, whose hard limits are 145 characters on the front and 185 on the back, while the corpus tops out at 66 and 154. This is a fixed expectation, not a golden value to re-measure — and it is the assertion that catches a grid reaching the compile call but not the overflow query.
...
- **FR-006**: The press sheet MUST derive card width from `(210 mm − 2 × margin) ÷ columns` and card height from `(297 mm − 2 × margin) ÷ rows`, so card size, mirroring, crop marks and pagination all follow from the two numbers, as they do today.
...
- **FR-011**: The card MUST NOT be typeset smaller at a denser grid. Reading text stays 11 pt and the front prompt 14 pt; `scale` stays 1.0. A card that does not fit is reported and split by the author, never rescaled.
...
- **FR-016**: `skills/cards/SKILL.md` MUST write the `grid:` key into the decks it produces and MUST size its card text to the declared grid rather than to a single assumed size.
...
- **FR-023**: The head-band label-length check MUST be a **warning**, not an error. 11 of the 38 cards shipped in this repo already exceed the A7 budget, so an error would fail the existing gates on content this feature is not fixing. *(Found by CHK033.)*
...
- **The fixed bands**: `head-h` (8.6 mm) and `foot-h` (6.2 mm) are absolute. They only eat proportionally more of the card when *rows* change — and **both supported grids keep 4 rows**, so the bands stay at 20.6 % of card height at A7 and A8 alike. The vertical half of this worry does not arise in the shipped set. What A8 does cost is head-band label width: 91.4 mm → 41.4 mm, 45 % of today, with `clip: true` so the loss is silent. That is a design judgement to make on paper, and SC-007 gates on it.
...
- **SC-003**: At `--margin 0`, `--grid a7` cuts to exactly 105 × 74.25 mm and `--grid a8` to exactly 52.5 × 74.25 mm — DIN A7 and DIN A8, so both fit boxes sold for those sizes.
...
- **SC-005**: At both A7 and A8 the deliberately-overlong card in `tests/fixtures/demo-project/broken/overflowing.yaml` is reported by id, and **no** demo card is reported at either grid. Both halves are needed: the first alone passes even if the overflow query ignores the grid, because that card overflows at both sizes.
...
- **SC-007**: **Release gate.** An A8 sheet printed duplex on real card stock has every back behind its front, cuts cleanly on the crop marks to 50 × 71.75 mm, and a cut card drops into a DIN A8 Lernbox. For the head band: a label **within the ~22-character A8 budget** is complete and legible at 6 pt, and an over-budget label clips cleanly at the band edge without disturbing the layout. This cannot be judged from a PDF viewer and blocks the merge. **A short-label card must be printed alongside the demo corpus** — all 38 cards shipped in this repo exceed the A8 budget, so the demo project alone cannot demonstrate the unclipped case.
