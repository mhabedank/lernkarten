...
- [X] T039 *(reopened by [BUG-011](bugs/BUG-011.md), closed again 2026-09-08 — **all three engines measured**. Chromium via `--dump-dom`, Firefox headless via a screenshotted measurement banner, Safari via `safaridriver`, which serves plain WebDriver and needs no Selenium but does need *Allow remote automation* switched on by hand. Link row −23 → 0 in all three; the `menu` control −21 → 0, which is the half T039 found and the report had missed.)* Repeat T036 in the three engines the spike covered — Chromium, Firefox and Safari. *(~~All three pass~~ — **struck 2026-09-08 by [BUG-011](bugs/BUG-011.md)**: this pass certified a bar whose link row hung 23 px too high, so it recorded that the row was *run*, not that it was *right*. Chromium measured headless, Safari by hand, Firefox 154 after it was installed. Firefox also supplied the 360 px measurement Chrome's 500 px window clamp had made impossible — see research.md R2.)* CI has no browser leg and will not grow one for this feature, so this is the only place the cross-browser claim is checked
...
- [X] T053 *(done 2026-09-08 in all three engines — see T039. This row is what found the mobile half: the desktop clause passed everywhere, and the control did not.)* Re-run reopened **T036** and **T039** against the corrected
...
- [X] T056 The `menu` control below the breakpoint, found while closing T039 in Safari and **not** in the original report: it hangs 21 px too high for the same reason the links did, from the same commit, and against T010's own instruction that the control get "the same label treatment the existing nav links have". `display: flex` therefore moves from the `min-width: 761px` block onto `.nav__menu` unscoped, where it serves both sides; only `::details-content` stays desktop-only. A10 widens with it — scoping the assertion to the desktop block is what let this half through *(FR-005, FR-018, SC-011)*
