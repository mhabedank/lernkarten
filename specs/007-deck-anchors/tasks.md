...
- [ ] T032 Checkpoint — `python3 scripts/check_docs.py` exits 0; `git diff skills/` shows **no** change inside any `---` frontmatter block; every relative link added by T027–T031 resolves. Then read all three prompts cold and confirm SC-006 and SC-008 by the criteria in `quickstart.md` §8.
- [ ] T033 Commit in two: `skill: state the anchor rule and read depth as a ceiling` for `skills/*/SKILL.md`, then `docs: document the Term: line and the two new checks` for `CLAUDE.md` and `docs/testing.md`.
