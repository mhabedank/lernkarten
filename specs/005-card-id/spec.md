...
- **FR-008**: `scripts/check_project.py` MUST report two cards in one project sharing an id, and the message MUST name **both** cards. Sharing is judged on the **normalised** id (FR-004), so `a45dk` and `A45DK` are a duplicate.
...
- **FR-013a**: The two paths are enumerated, not described. The **writing** path is `lernkarten id --backfill` and `lernkarten id --reassign`, and only those. The **validating** path is `lernkarten check` and `scripts/check_project.py`; `lernkarten build` belongs to neither — it reads and renders ids but never writes them. The validating path MUST NOT reassign: they are read-only CI gates, and a gate that rewrote the working tree would be unusable as one. They report the duplicate and exit non-zero (FR-008), leaving the fix to the writing path.
