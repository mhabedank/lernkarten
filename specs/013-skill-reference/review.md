...
**Feature**: Sphinx documentation foundation (013)
...
| Feasibility & Risk | **FAIL** | T041's workflow carries no `_site/docs` token, so T042's adapted test is red after T041 and a comment would satisfy it (C2); rows 7 and 10 write into the real `docsite/` (W3); T015's `python3` fails the Windows leg (W4) |
...
- [ ] **W4** — T015: `sys.executable`, not `python3`, in the subprocess call.
