...
| 4a | `/sources` | in a scratch copy that has a `goal.md`, `/sources ~/lernkarten-demo/raw/field-notes` | **run output (FR-001, FR-005):** one goal-fit statement that names a required topic or area of `goal.md` by name, and says which of `kind` / `depth` it reasoned from — not merely that it weighed the source. **FR-009 is circular and this row says so**: where the run judged from the URL, the `note` and the type without looking at the material, it has to say it did. A tester cannot tell a looked-at claim from an unlooked-at one except by asking the run, and the automated gate in `check_docs.py` only asserts that the *rule is stated* in the prompt. Read this half as "the run declares what it went on", never as "the run was checked for honesty" |
| 4b | `/sources` | in the scratch copy edit **one word** of `goal.md` — `depth: working` → `awareness` — then register practitioner material the goal does not ask for | **run output and on disk (FR-002, FR-003, FR-005; SC-002, SC-004):** the warning names the source `id` **and** quotes the `goal.md` line it conflicts with, never a bare "looks off-goal"; it says the material was weighed **down** and which of `kind` / `depth` did it; the entry is in `sources.yaml` when the run ends, written with **zero** confirmation prompts in between. Never edit `tests/fixtures/` for this |
| 4c | `/sources` | the same one word again, `awareness` → `expert`, and register the same material | **run output (FR-005):** the same material is weighed **up**, and the run *says so*. Read 4b and 4c side by side: FR-005 wants the run to name which of `kind` / `depth` it used, so the two runs must differ in their **stated reason**, not only in their verdict |
| 4d | `/sources` | read the warning from 4b or 4f closely, sentence by sentence | **run output (FR-004):** every sentence is a judgement about *this source for this goal*. Nothing says the subject is unimportant, the publisher unreliable or the source poor in general. A sentence you could lift out of the run and still read as a verdict on the publisher fails the row (constitution VII) |
| 4e | `/sources` | `/sources` with no argument, in the scratch copy that still has `goal.md` | **run output (FR-008):** every source listed, and **zero** goal-fit statements — no verdict, no warning, nothing re-derived. The assessment belongs to registration |
| 4f | `/sources` | add an `## Out of scope` line to the scratch `goal.md`, then register a source that matches it | **run output (FR-003, SC-002):** the warning names the source id and **quotes that line**; **zero** warnings say only that something looks off-goal. The source is registered anyway (FR-002) |
| 4g | `/sources` | after any registration in a project with `goal.md`, read `sources.yaml` and run `python3 scripts/check_project.py <project> --strict` | **on disk (FR-007, SC-001):** the entry carries exactly the keys its type carried before this feature — no `fit:`, no `verdict:`, no score, no comment. The verdict lived in the run and nowhere else; `--strict` exits 0 |
| 4h | `/sources` | register a practitioner archive as `type: web` with `depth: 1` | **run output (FR-029):** at registration the run states plainly what the fetch will reach — the index page plus the posts on the same domain linked from it, capped at 20 by `skills/ingest/SKILL.md` — so a ten-year archive is visibly *not* fully ingested. No sixth source type is offered and `depth:` is unchanged |
| 4i | `/sources` | `rm goal.md` in the scratch copy, then register a source | **run output and on disk (FR-006, SC-003):** no assessment and no warning; the entry's key set is identical to the one today's `/sources` writes; **at most one** line in the whole run points at `/learning-goal`, never one per source; `check_project.py <project> --strict` exits 0 and reports nothing new |
| 4j | `/sources` | with no `goal.md`, register two sources; **then** write `goal.md` with `/learning-goal` and read that run | **run output (research R1):** the goal run assesses nothing that is already registered and claims nothing about it — no line implies the register has been re-judged. The assessment is attached to registration, so material registered before the goal existed stays unjudged, and re-registering by hand is the only way to get a verdict on it |
| 4k | `/sources` | `/sources --discover` in the scratch copy with a `goal.md` | **run output (FR-016 – FR-018, FR-027, FR-035, FR-039, FR-040; SC-007, SC-017):** each candidate says what it is and where it lives, **which class of material it is**, the goal topic or area it would serve, one credibility sentence naming what the source is *and what it is not*, and an entry ready to register with a known `type` and a kebab-case `id`. **Zero** scores, ratings, percentages or star counts. **Every** candidate names a class, the non-practitioner ones included, and **zero** are dropped or renamed for naming a class no list holds — there is no list, no class argument and no way to ask for one class. Nothing is written until you pick. **Of FR-027, what is decidable**: the *shown* count equals the number of candidates listed, every goal area appears including the ones nothing was found for, and *found* ≥ *shown*. **What is not**: the *found* number itself — you see what the run printed and have nothing to check it against, so record it as **read, not verified** |
| 4s | `/sources` | in the same proposal list, read the credibility sentence of a practitioner candidate (an incident post-mortem, a case study, a company engineering blog) beside one that is not | **run output (FR-019, SC-007) — the practitioner addendum**, read next to 4k: the practitioner candidate's sentence *additionally* states both properties — that a company's account of its own incident is a primary source **and** an interested one, and that material of this kind is published only by the parties who came through the incident, so the cases that ended badly are not among what can be found. It has to **state** them, not name them: "a selected sample" standing alone does not do it. The non-practitioner candidate is held to **neither** property and is not faulted for missing them |
| 4l | `/sources` | run `/sources --discover` and decline every candidate | **on disk (FR-016, SC-005):** `sources.yaml`, `knowledge/` and `catalog/` are unchanged — **zero** files created, **zero** modified. Judge it with `git status` in the scratch copy, not by eye |
| 4m | `/sources` | run `/sources --discover` again and accept exactly one candidate | **run output and on disk (FR-020, FR-022, SC-006):** the pick goes through the **ordinary** registration path, so the goal-fit statement of row 4a fires for it exactly as for a source you named; the entry lands in `sources.yaml`; **no** `type: research` entry and **nothing** written into `knowledge/` — synthesising documents is `/research-gaps`'s job, not discovery's; `python3 scripts/check_project.py <project> --strict` exits 0 |
| 4n | `/sources` | with the candidate accepted in 4m still registered, run `/sources --discover` again for the same goal | **run output (FR-024):** that source is **not** proposed a second time. This is the one of discovery's three exclusions a tester can produce on demand — the other two are row 4n-i's, and they are not |
| 4n-i ⚠ | `/sources` | open the URL of **every** candidate a discovery run proposed | **opportunistic, and it says so (FR-021, FR-023).** The stimulus is not yours to produce: discovery cannot be made to *find* an unretrievable or a paywalled candidate. A candidate that 404s, that sits behind a paywall or a login, or whose page does not match its description **is a violation** and the row **fails**. If none of the proposals is any of those, the row is **not evidence of conformance** — write "not exercised", never "pass". It can detect a violation; it cannot confirm compliance. The prompt-side halves are gated separately in `scripts/check_docs.py` |
| 4o | `/sources` | `rm goal.md`, then `/sources --discover` | **run output (FR-025):** it says it has nothing to search for, points at `/learning-goal`, and writes nothing — **zero** candidates invented out of the register |
| 4p | `/sources` | turn the network off, then `/sources --discover` | **run output (FR-026, SC-008):** it reports that it could not search, points at the ordinary registration path for material you already have, writes nothing and exits cleanly — **no traceback**. The `/research-gaps` twin of this row is 9d |
| 4q | `/sources` | with `goal.md` present and the network on, run a registration, a bare listing and a removal — asking for discovery in none of them | **run output (FR-036, SC-015):** discovery is mentioned **nowhere** in any of the three — no candidate, no proposal, no closing line offering to go looking. Discovery exists; asking for it at invocation is the only way in |
| 4r | `/sources` | with the machine **offline** (the same switch row 4p uses), run the same registration, the same bare listing and the same removal again | **the observable is the network, not the transcript (FR-033).** All three behave **exactly** as they did online: the same entry written, the same listing, the same removal, no error and no "could not reach" line. A run that needs the network to register a source you named **fails** this row. Read the transcript's tool calls as well — zero WebFetch, WebSearch or fetch of any kind — but the offline run is what decides it, because a transcript is a report and the network is a fact |
| 5 | `/ingest` | `/ingest field-notes` on the raw demo copy | nine files under `knowledge/field-notes/` — including the one from `appendix/`; the umlaut file name becomes a clean slug; the empty file is reported, not invented |
...
| 8m ⚠ | `/ingest` | `/ingest field-notes`, then `/ingest handbook`, then read the frontmatter of the two documents | **on disk (FR-015):** the incident write-up (`knowledge/field-notes/torvig-radio-outage.md`) carries `nature: experience`; the handbook document carries **no** `nature:` key at all. Absent is not a finding — `nature:` is optional and additive, and `check_project.py --strict` says nothing about a document without it. ⚠ the *classification* is the run's judgement and no check can second-guess it; what this row decides is that the recognition reached disk in the right shape, with the right value, on the right document |
...
| 9f | `/catalog` | `/catalog` over a subtopic whose only references are experience reports — in the demo project, *The Torvig radio outage of 12 March* | **run output (FR-013, FR-014):** the material-base warning carries **all four** of FR-013's contents, in the run's own words — (1) the subtopic named and what it rests on, with nothing general among it; (2) *why* that base is skewed, written out rather than named; (3) what that means for the cards; (4) what would balance it. No phrase is prescribed, and a run saying only "published incidents are a selected sample" does **not** satisfy it. And FR-014: the report says the required topic wants a general rule and only cases cover it — single-case coverage is never presented as coverage of the rule. Advisory throughout: never an error, never a reason to withhold a subtopic, and never a suggestion to run discovery |
...
| 12-iv | `/cards` | `/cards` over that same experience-only subtopic, and read the closing report | **run output (FR-013) — the `/cards` half**: the warning carries the same four contents as row 9f, in this run's own words. FR-013 binds **both** skills, so a `/cards` run that reports on the subtopic without it is incomplete even when `/catalog` said it earlier. Advisory: no card is withheld because of it |
| 12-v | `/cards` | read the cards that run wrote | **on disk (FR-011, FR-012; SC-009):** **every** card names the case it came from — the incident in the text, the attribution in the optional `source:` key — and **zero** state an unattributed general rule resting on that one case. Where the fact depends on the scale or the circumstances of the case, the card carries that context instead of dropping it |
| 12-vi | any | a full `/sources` register → `/ingest` → `/catalog` → `/cards` run with **no** `--discover` anywhere; then, in the same session, a `/learning-goal` run and a `/research-gaps` run | **run output and on disk (FR-037, FR-038; SC-014, SC-015):** **zero** lines across the whole run mention discovery, propose a candidate or offer to go looking — `/catalog`'s FR-014 report and FR-013's material-base warning included, and a `Status: gap` subtopic's one pointer stays `/research-gaps` as today. **Zero** entries in `sources.yaml` beyond the ones you named. The last two runs are the point: `/learning-goal` is held by the `--discover` token gate but its *paraphrase* is not, and `/research-gaps` **must** carry the token for FR-034's seam, so no token gate can bound it. Neither may end with a line offering to go looking |
...
| | the default (`--grid a8`) | `--grid a7` |
|---|---|---|
| 17 registration | 5 vertical, 5 horizontal cut lines | 3 vertical, 5 horizontal cut lines |
| 18 cut card | 71.75 × 50 mm (74.25 × 52.5 at `--margin 0`) | 100 × 71.75 mm (105 × 74.25 at `--margin 0`) |
| 19 borderless | drops into a DIN A8 box | drops into a DIN A7 box |
